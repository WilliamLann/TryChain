...
public class ETHAccountDeal {
	...
	public static void main(String[] args) throws Exception {
		LynxUtil.startARP();
		ETHAccountDeal deal = new ETHAccountDeal();
		deal.dealAccount();
	}
	...
	public  void dealAccount() throws Exception {
		...
		log.info("开始处理eth account的balance");
		DefaultBlockParameter defaultBlockParameter = DefaultBlockParameter.valueOf("latest");
		List<EthAddress> adds = EthAddress.dao.find("select * from eth_address order by (value+0) desc limit 20");
		for (int i = 0; i < adds.size(); i++) {
			try {
				EthAddress add = adds.get(i);
				add.setEthHoldValue(web3j.ethGetBalance(add.getEthAddress(), defaultBlockParameter).send().getBalance().toString());;
				add.update();
			}catch(Exception e) {
